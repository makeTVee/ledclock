...
uint8_t sys_state = 1;     // state machine variable for different modes (LED only, clock, timer ...)
uint8_t hue = 0;
...
void loop()
{
  ...
  switch (sys_state)
  {
  case 0: // inner LEDs only, no ring
    FastLED.clear();
    FastLED.show();
    break;
  case 1: // outer ring clock
    if (transition)
    {
...
void set_time(uint8_t hour, uint8_t minute, uint8_t second)
{
  ...
  // three red LEDs for hour
  leds[(hour % 12) * 5 + (minute / 12)] = CRGB::Red;
  leds[(((hour % 12) * 5 + (minute / 12)) + 1) % 60] = 0x00300000; // dim red (0x00RRGGBB)
  leds[(((hour % 12) * 5 + (minute / 12)) - 1) % 60] = 0x00300000;

  // two green LEDs for minute
  leds[minute] += CRGB::Green;
  leds[(minute - 1) % 60] += 0x00003000; // dim green (0x00RRGGBB)

  // one blue LED for second
